...
if $opt_local; then
    ...
    if $opt_debug; then
        INSTALLER="$SCRIPTDIR/OmarchyMediaWriter-win32-${VERSION_FULL}-debug.exe"
    else
        INSTALLER="$SCRIPTDIR/OmarchyMediaWriter-win32-${VERSION_FULL}.exe"
    fi
else
...
if ! $opt_nosign; then
    osslsigncode sign -pkcs12 $CERTPATH/authenticode.pfx -readpass "$CERTPASS" -h sha256 -n "Omarchy Media Writer" -i https://omarchy.org -t http://timestamp.comodoca.com/authenticode -in "$MEDIAWRITER" -out "$MEDIAWRITER.signed" >/dev/null
    mv "$MEDIAWRITER.signed" "$MEDIAWRITER"

    osslsigncode sign -pkcs12 $CERTPATH/authenticode.pfx -readpass "$CERTPASS" -h sha256 -n "Omarchy Media Writer" -i https://omarchy.org -t http://timestamp.comodoca.com/authenticode -in "$HELPER" -out "$HELPER.signed" >/dev/null
...
if ! $opt_nosign; then
    osslsigncode sign -pkcs12 $CERTPATH//authenticode.pfx -readpass "$CERTPASS" -h sha256 -n "Omarchy Media Writer" -i https://omarchy.org -t http://timestamp.comodoca.com/authenticode -in "$INSTALLER" -out "$INSTALLER.signed" >/dev/null
